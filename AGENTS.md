...
- **Document Versioning:** Documents support branching history similar to chats via a `parent_id` foreign key tracked natively in SQLite. When performing non-destructive AI editing operations (like "Replace in place"), the UI clones the old state into a new sub-document version recursively before mutating the child.
...
- **Error Handling Architecture:** All asynchronous and synchronous AI requests strictly enforce emitting `onError` blocks (or signals from `QNetworkReply`) wrapped in `KMessageBox` to notify users dynamically while actively guarding against overwriting GUI states for active edits upon failure. In Queue jobs, errors trigger a "Cancel" to "Send" toggle state.
